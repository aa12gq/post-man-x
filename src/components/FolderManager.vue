<template>
	<div class="folder-manager">
		<div
			v-for="folder in workspaceStore.collectionList"
			:key="folder.id"
			class="folder-item"
			:data-folder-id="folder.collection_id"
			@dragover.prevent
			@drop="onDrop"
		>
			<div class="folder-header">
				<div
					class="folder-info"
					@click="toggleFolder(folder.collection_id)"
				>
					<el-icon
						class="expand-icon"
						:class="{
							'is-expanded': isExpanded(folder.collection_id),
						}"
						v-if="hasChildren(folder.collection_id)"
					>
						<CaretRight />
					</el-icon>
					<el-icon><Folder /></el-icon>
					<div
						v-if="!isEditing(folder.collection_id)"
						class="folder-name"
						@dblclick="startEditing(folder.collection_id)"
					>
						{{ folder.name }}
					</div>
					<div
					...
					>
						<el-input
							...
							ref="editInput"
							v-focus
							@keyup.enter="finishEditing(folder.collection_id)"
						...
						<div class="editing-actions">
							<el-button
								...
								circle
								@click.stop="
									finishEditing(folder.collection_id)
								"
							>
			...
			<div
				v-if="isExpanded(folder.collection_id)"
				class="folder-content"
			>
				<div
					v-for="subFolder in workspaceStore.handleGetFolderList(
				...
				>
					<div class="folder-header">
						<div
							class="folder-info"
							@click="toggleFolder(subFolder.folder_id)"
						>
							<el-icon
								class="expand-icon"
								:class="{
									'is-expanded': isExpanded(subFolder.folder_id),
								}"
								v-if="hasChildren(subFolder.folder_id)"
							>
								<CaretRight />
							</el-icon>
							<el-icon><Folder /></el-icon>
							<div
								v-if="!isEditing(subFolder.folder_id)"
								class="folder-name"
							...
							>
								<el-input
									...
									ref="editInput"
									v-focus
									@keyup.enter="finishEditing(subFolder.folder_id)"
									@keyup.esc="cancelEditing"
								/>
								<div class="editing-actions">
									<el-button
										...
										circle
										@click.stop="
											finishEditing(subFolder.folder_id)
										"
									>
										<el-icon><Check /></el-icon>
									</el-button>
							...
							>
								<el-icon>
									<component
										...
										"
										:style="{
											color: getRequestIcon(request.type)
												.color,
										}"
									/>
								</el-icon>
								<div
									v-if="!isEditingRequest(request.id)"
									class="request-name"
								>
								...
								>
									<el-input
										...
										v-focus
										@keyup.enter="
											finishEditingRequest(request.id)
										"
										@keyup.esc="cancelEditing"
									/>
									<div class="editing-actions">
					...
					>
						<el-icon>
							<component
								...
								:style="{
									color: getRequestIcon(request.type).color,
								}"
							/>
						</el-icon>
						<div
...
<script setup lang="ts">
	...
	const handleRequestCommand = async (
		command: string,
		request: HistoryItem
	) => {
		switch (command) {
